...
const GUIDES: Guide[] = [
  ...
  },
  {
    ...
    stepsEn: [
      ...
      'To view records: choose the Company / Site / Period / Dimension / Theme / Metric / Status filters, then click "Search / Load Records" — only the selected scope is loaded, so the page stays fast.',
    ...
    stepsTh: [
      ...
      'ดูรายการ: เลือกตัวกรอง บริษัท / สถานที่ / รอบ / มิติ / หัวข้อ / ตัวชี้วัด / สถานะ แล้วกด "ค้นหา / แสดงข้อมูล" — ระบบดึงเฉพาะขอบเขตที่เลือก ทำให้หน้าโหลดเร็ว',
  ...
  },
  {
    ...
    introEn: 'A dedicated greenhouse-gas dashboard: Scope 1 / 2 / 3 breakdown and a Net-Zero trajectory.',
    introTh: 'แดชบอร์ดก๊าซเรือนกระจกโดยเฉพาะ: แยก Scope 1 / 2 / 3 และเส้นทางสู่ Net Zero',
    stepsEn: [
      'KPI cards show the YTD total plus Scope 1, 2 and 3 separately, and the Year-over-Year change (negative = emissions fell, good).',
      'The donut shows each scope’s share; charts are colour-coded red = Scope 1, amber = Scope 2, blue = Scope 3.',
      'Net-Zero Trajectory: stacked bars per year by scope; the dashed horizontal line is the annual target cap (“below X”).',
      'Monthly (YTD) and By-Site stacked charts break it down; the monthly chart’s dashed line is the average monthly target.',
      'Hover the ⓘ icon on any card for a short explanation of how to read it.',
    ],
    stepsTh: [
      'การ์ด KPI แสดงยอดรวม YTD และแยก Scope 1, 2, 3 พร้อมการเปลี่ยนแปลงเทียบปีก่อน (YoY) — ค่าติดลบ = ปล่อยลดลง (ดี)',
      'โดนัทแสดงสัดส่วนแต่ละ Scope · กราฟใช้สี แดง = Scope 1, เหลือง = Scope 2, น้ำเงิน = Scope 3',
      'Net-Zero Trajectory: แท่งซ้อนรายปีแยกตาม Scope · เส้นประแนวนอน = เพดานเป้าหมายทั้งปี (“below X”)',
      'กราฟรายเดือน (YTD) และรายสถานที่ช่วยเจาะลึก · เส้นประในกราฟรายเดือน = เป้าเฉลี่ยต่อเดือน',
      'เลื่อนเมาส์ที่ไอคอน ⓘ บนแต่ละการ์ดเพื่อดูคำอธิบายวิธีอ่านกราฟ',
    ],
    tipTh: 'ค่า GHG มาจากการคำนวณอัตโนมัติ (กิจกรรม × Emission Factor) — ตั้งค่า EF/Scope/Auto ได้ที่เมนู "ตั้งค่า GHG"',
    tipEn: 'GHG figures are auto-calculated (activity × emission factor) — configure EF / scope / auto in the "GHG Settings" menu.',
  },
  {
  ...
  },
  {
    ...
    introEn: 'Set each activity’s emission factor by Scope 1/2/3 and choose auto/manual GHG calculation per activity.',
    introTh: 'ตั้งค่า Emission Factor ต่อกิจกรรม แยกตาม Scope 1/2/3 และเลือกคำนวณ GHG แบบอัตโนมัติ/กรอกเอง รายกิจกรรม',
    stepsEn: [
      'Build a reference library: download the Template, fill the emission factors, and Import Reference (Excel). This is a lookup only — keyed by activity_code.',
      'The activity table is grouped by Scope 1 / 2 / 3. Each row shows the closest reference match (hover for source); click ✨ to apply it, or type the EF yourself.',
      'Click Save per row (or "Save all"); only saved values feed the calc. "Restore to default" loads reference values into the fields (you still Save).',
      'Per activity, switch GHG entry to Auto (system computes activity × factor) or Manual; "Auto all" turns on every activity that has a saved EF.',
      '"Backfill GHG" computes GHG from all existing historical activity data — without overwriting values you already have.',
    ],
    stepsTh: [
      'สร้างคลังอ้างอิง: กด Template → กรอกค่า EF → "นำเข้าอ้างอิง (Import Reference)" (Excel) ใช้เป็นตัวอ้างอิงเท่านั้น จับคู่ด้วย activity_code',
      'ตารางกิจกรรมแบ่งตาม Scope 1 / 2 / 3 · แต่ละแถวแสดงค่าอ้างอิงที่ใกล้เคียงสุด (เลื่อนเมาส์ดูที่มา) · กด ✨ เพื่อใช้ค่านั้น หรือพิมพ์ EF เอง',
      'กด Save ทีละแถว (หรือ "บันทึกทั้งหมด") — เฉพาะค่าที่ saved เท่านั้นที่ใช้คำนวณ · "คืนค่าเริ่มต้น" โหลดค่าอ้างอิงเข้าช่อง (ยังต้องกด Save)',
      'แต่ละกิจกรรมสลับ "คำนวณ GHG" เป็น Auto (ระบบคูณ กิจกรรม × factor) หรือ Manual · "เปิดอัตโนมัติทั้งหมด" เปิดทุกกิจกรรมที่บันทึก EF แล้ว',
      '"คำนวณย้อนหลัง (Backfill)" คำนวณ GHG จากข้อมูลกิจกรรมเก่าทั้งหมด โดยไม่เขียนทับค่าที่มีอยู่แล้ว',
    ],
    tipTh: 'ค่า EF ที่ match อ้างอิงต่ำกว่า 60% จะไม่ใส่ค่าให้อัตโนมัติ (กันค่าผิด) · GHG จะเป็น Draft จนกว่ากิจกรรมที่เกี่ยวข้องจะถูก Submit',
    tipEn: 'Low-confidence reference matches (<60%) are never auto-applied (prevents wrong values) · GHG stays Draft until its activities are Submitted.',
  },
  {
